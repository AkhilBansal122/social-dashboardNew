...
<div>
    {{-- ── Flash from session (OAuth redirects) ── --}}
    @if (session('success'))
    ...
    {{-- ── Inline sync messages ── --}}
    @foreach ($messages as $plat => $msg)
        <div class="flash {{ $msg['type'] === 'success' ? 'flash-s' : 'flash-e' }}" style="margin-bottom:.75rem">
            <span>{{ $msg['text'] }}</span>
            <button class="flash-x" wire:click="clearMessage('{{ $plat }}')">✕</button>
        </div>
    @endforeach
    ...
    <div class="plat-grid">
        @foreach ($this->platforms as $platform)
            @php $account = $this->connectedAccounts->get($platform); @endphp

            @if ($account)
                <div class="plat-card">
                    {{-- Card header --}}
                    <div class="plat-head {{ $platform === 'instagram' ? 'ig' : 'snap' }}">
                        <div class="plat-name">
                            <div class="plat-icon {{ $platform === 'instagram' ? 'ig' : 'snap' }}">
                                {{ $platform === 'instagram' ? '📸' : '👻' }}
                            </div>
                            {{ $account->platformLabel() }}
                        </div>
                        <span class="badge {{ match($account->status) { 'active' => 'badge-green', 'expired' => 'badge-amber', 'error' => 'badge-red', default => 'badge-muted' } }}">
                            <span class="dot"></span>{{ $account->statusLabel() }}
                        </span>
                    </div>
                    ...
                    <div class="plat-body">
                        ...
                        {{-- Status alert for expired / error --}}
                        @if ($account->status === 'expired')
                            <div class="card-msg msg-w">
                                ⚠ {{ $account->status_message ?? 'Token expired — please reconnect.' }}
                            </div>
                        @elseif ($account->status === 'error')
                            <div class="card-msg msg-e">
                                ✗ {{ $account->status_message ?? 'An error occurred.' }}
                            </div>
                        @endif
                        ...
                        @if ($account->profile)
                            <div class="profile-row">
                                <img src="{{ $account->profile->getAvatarUrl() }}"
                                     alt="avatar"
                                     class="avatar"
                        ...
                        {{-- Actions --}}
                        <div class="actions">
                            @php $syncing = $platform === 'instagram' ? $syncingInstagram : $syncingSnapchat; @endphp

                            <button class="btn btn-primary btn-sm"
                                    wire:click="syncPlatform('{{ $platform }}')"
                                    @if ($account->status === 'expired' || $syncing) disabled @endif>
                                @if ($syncing)
                            ...
                            @if (in_array($account->status, ['expired', 'error']))
                                <a href="/auth/{{ $platform }}"
                                   class="btn btn-sm {{ $platform === 'instagram' ? 'btn-ig' : 'btn-snap' }}">
                                    🔗 Reconnect
                                </a>
                            @endif

                            <button class="btn btn-danger btn-sm"
                                    wire:click="disconnect('{{ $platform }}')"
                                    wire:confirm="Disconnect {{ $account->platformLabel() }}? This will remove all synced data for this account.">
                                ✕ Disconnect
                            </button>
                        </div>

                        @if ($account->last_synced_at)
                            <div class="sync-meta">Last synced {{ $account->last_synced_at->diffForHumans() }}</div>
                        @endif
                    </div>
                </div>
                ...
                <div class="plat-card">
                    ...
                    <div class="plat-body">
                        <div class="empty-card">
                            <p>Connect your {{ ucfirst($platform) }} account to start syncing your content and profile data.</p>
                            <a href="/auth/{{ $platform }}"
                               class="btn {{ $platform === 'instagram' ? 'btn-ig' : 'btn-snap' }}">
    ...
    @if ($this->instagramAccount)
        ...
        @if ($this->instagramAccount->isActive())
            <div class="media-header">
                <div>
                    <div class="media-title">📸 Instagram Posts</div>
                    @if ($this->instagramMedia && $this->instagramMedia->total() > 0)
                        <div class="media-count">{{ number_format($this->instagramMedia->total()) }} posts synced</div>
                    @endif
                </div>
                <div class="media-controls">
                    <button class="view-btn {{ $mediaView === 'grid' ? 'on' : '' }}" wire:click="setView('grid')">⊞ Grid</button>
                    <button class="view-btn {{ $mediaView === 'list' ? 'on' : '' }}" wire:click="setView('list')">☰ List</button>
            ...
            @if ($this->instagramMedia && $this->instagramMedia->count() > 0)

                @if ($mediaView === 'grid')
                    <div class="mgrid">
                        @foreach ($this->instagramMedia as $m)
                            <div class="mcard">
                                <div class="mthumb-wrap">
                                    @if ($m->getThumbnail())
                                        <img src="{{ $m->getThumbnail() }}" alt="" class="mthumb"
                                             onerror="this.style.display='none';this.nextElementSibling.style.display='grid'">
                                        <div class="mthumb-ph" style="display:none">🖼️</div>
                                    @else
                                        <div class="mthumb-ph">🎥</div>
                                    @endif
                                    <span class="mtype">{{ $m->mediaTypeIcon() }}</span>
                                </div>
                                <div class="minfo">
                                    <div class="mcap">{{ $m->getCaptionSnippet(85) ?: '—' }}</div>
                                    <div class="mstats">
                                        <span class="mstat">❤️ {{ number_format($m->like_count) }}</span>
                                        <span class="mstat">💬 {{ number_format($m->comments_count) }}</span>
                                        @if ($m->hasInsights())
                                            <span class="ins-badge">👁 {{ number_format($m->impressions) }}</span>
                                        @endif
                                    </div>
                                    @if ($m->posted_at)
                                        <div class="mdate">{{ $m->posted_at->format('M j, Y') }}</div>
                                    @endif
                                </div>
                            </div>
                        @endforeach
                ...
                @else
                    <div class="mlist">
                        @foreach ($this->instagramMedia as $m)
                            <div class="mrow">
                                @if ($m->getThumbnail())
                                    <img src="{{ $m->getThumbnail() }}" alt="" class="mrow-thumb"
                                         onerror="this.style.display='none';this.nextElementSibling.style.display='grid'">
                                ...
                                @endif
                                <div class="mrow-body">
                                    <div class="mrow-cap">{{ $m->getCaptionSnippet(160) ?: '(no caption)' }}</div>
                                    <div class="mrow-meta">
                                        <span>{{ $m->mediaTypeIcon() }} {{ str_replace('_', ' ', $m->media_type) }}</span>
                                        <span>❤️ {{ number_format($m->like_count) }}</span>
                                        <span>💬 {{ number_format($m->comments_count) }}</span>
                                        @if ($m->hasInsights())
                                            <span class="ins-badge">👁 {{ number_format($m->reach) }} reach · {{ number_format($m->impressions) }} imp.</span>
                                        @endif
                                    </div>
                                </div>
                                <div class="mrow-date">{{ $m->posted_at?->format('M j, Y') }}</div>
                            </div>
                        @endforeach
                    </div>
                @endif
                ...
                <div class="pager">
                    <div class="pager-inner">
                        {{ $this->instagramMedia->links('livewire.pagination') }}
                    </div>
                </div>
        ...
        @elseif ($this->instagramAccount->status === 'expired')
            <div class="empty">
                <div class="empty-ico">⏰</div>
                <h3>Instagram token expired</h3>
                <p>{{ $this->instagramAccount->status_message ?? 'Your Instagram session has expired.' }}</p>
        ...
        @else
            <div class="empty">
                ...
                <p>{{ $this->instagramAccount->status_message ?? 'Please reconnect your Instagram account.' }}</p>
                <div style="margin-top:1rem">
                    <a href="/auth/instagram" class="btn btn-ig">🔗 Reconnect Instagram</a>
                </div>
            </div>
